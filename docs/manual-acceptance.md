...
这份清单只用于人工验收。不要把这些步骤放进自动测试、CI、脚本或 agent 默认验证。
...
- [ ] 复制多文件 URL，历史列表保留文件信息，右侧显示数量和路径，重新选择后目标 App 能收到文件 URL。
- [ ] 复制图片会新增历史记录，列表轻量显示“图片”，右侧可看到缩略图预览。

## 面板与搜索

- [ ] 快捷键呼出面板，窗口居中显示。
- [ ] 重新呼出时默认回到最近历史，不保留上次搜索词。
...
- [ ] warning 阈值：pasteboard read > `50 ms`，Core insert > `50 ms`，capture total > `100 ms`。
